...
import type { DocLineItem } from './QuotationPDF'

// See QuotationPDF.tsx for why there's no Font.register() here — a remote
// font fetch was the root cause of "Gagal generate PDF".
      ...
      <Page size="A4" style={s.page}>
        ...
        <View style={s.metaRow}>
          ...
          <View style={s.metaBlock}>
            <Text style={s.metaLine}>Nomor Invoice: {invoiceNumber}</Text>
            <Text style={s.metaLine}>Tanggal: {tanggal}</Text>
            {dueDate && <Text style={[s.metaLine, { fontSize: 9, color: '#666' }]}>Jatuh tempo {dueDate}</Text>}
          </View>
        </View>
        ...
        <View style={s.table}>
          ...
          {items.map((item, i) => (
            <View key={i} style={s.tableRow}>
              ...
              <Text style={[s.descLine, s.colPrice]}>{item.is_bonus ? 'Bonus' : formatRp(item.price * item.qty)}</Text>
            </View>
          ))}
        </View>
