-- 1. Add pilar_konten column to videos table
ALTER TABLE public.videos ADD COLUMN pilar_konten TEXT;

-- 2. Update auto_assign_no_upload trigger function to NOT clear deadline_posting
...
BEGIN
  ...
  IF NEW.status IN ('scheduled', 'live') THEN
    -- Keep target date (deadline_posting) - DO NOT clear it anymore!
    -- NEW.deadline_posting := NULL; -- removed
...
-- 3. Update calibrate_workspace_videos to NOT clear deadline_posting
...
BEGIN
  ...
  -- 1. Renumber all scheduled/live videos in order of sort_order DESC (from the bottom up) - DO NOT clear deadline_posting
  FOR v_rec IN 
    SELECT id 
    FROM public.videos 
